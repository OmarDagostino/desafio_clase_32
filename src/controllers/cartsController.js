...
if (productsToWait.length > 0) {
  for (let n = 0; n < productsToWait.length; n++) {
    ...
    err=551;
    mesageError=errorDict.getErrorMessage(err);
    rres.status(err).send(mesageError);
  }
...
async function eliminarProductoDelCarrito (req, res) {
  try {
    ...
    if (!validObjectId) { 
      err=404;
      mesageError=errorDict.getErrorMessage(err);
      res.status(err).send("Identificador del carrito invalido");
      } else {
...
async function actualizarTodoElCarrito(req, res) {
  ...
  if (Array.isArray(nuevoCarrito.products) && nuevoCarrito.products.length > 0) {
    ...
    if (validacionExitosa.every((isValid) => isValid)) {
      ...
      await cartsServices.actualizarCarrito(cart,cartId);
      err=201;
      res.status(err).json({ mensaje: 'Carrito actualizado con éxito' });
    } else {
      err=453;
      mesageError=errorDict.getErrorMessage(err);
      res.status(err).json({ error: 'El contenido del carrito no es válido' });
    }
  } else {
    err=453
    res.status(err).json({ error: 'El contenido del carrito esta vacio o no es valido' });
  }
      }
   catch (error) {
    console.error(error)
